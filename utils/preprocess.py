...
def rela_recall_transfer(test_roidb, pred_roidb, N_recall):
	N_right = 0.0
	N_total = 0.0
	N_data = len(test_roidb)
	num_right = np.zeros([N_data, ])
	for i in range(N_data):
		# print(i)
		rela_gt = test_roidb[i]['rela_labels']
		if len(rela_gt) == 0:
			continue
		sub_gt = test_roidb[i]['sub_labels']
		obj_gt = test_roidb[i]['obj_labels']
		sub_box_gt = test_roidb[i]['sub_box']
		obj_box_gt = test_roidb[i]['obj_box']
...
def get_blob_pred_all(roidb_use, im_scale, index_sp, N_each_batch, batch_id):
	blob = {}
	sub_box = roidb_use['sub_box_gt']*im_scale
	obj_box = roidb_use['obj_box_gt']*im_scale
	rela = np.int32(roidb_use['rela_gt'])
	...
	# add 
	sub_ind_1=np.int32(roidb_use['sub_gt'])
	obj_ind_1=np.int32(roidb_use['obj_gt'])
	blob['sub_ind'] = sub_ind_1[index_use]
	blob['obj_ind'] = obj_ind_1[index_use]
	blob['img_path'] = roidb_use['image']
	blob['length'] = len(sub_box)
	return blob